...
@DatasetReader.register("rp_reader")
class TablesDatasetReader(DatasetReader):
    ...
    def __init__(self, lazy: bool = False,
        ...
        self.max_len = int(os.getenv('max_cell_len'))
    ...
    @overrides
    def _read(self, fn: str, max_rows=30, max_cols=4):
        with open(fn, "r") as data_file:
            for line in data_file:
                line = json.loads(line)
                header, cells = self.parse_table(line['table_data'], max_rows, max_cols)
                line['dtypes'] = line['dtypes'][:max_cols] if 'dtypes' in line else None

                # prepro labels
                max_label_idx = int(os.getenv('n_classes'))
                for k, lid in enumerate(line['label_idx']):
                    if int(lid) >= max_label_idx:
                        line['label_idx'][k] = 0
                        line['label'][k] = ''

                # skip blank line
                uniq_labels = list(set(line['label']))
                if len(uniq_labels) == 1 and uniq_labels[0] == '':
                    continue
                ...
                if len(cells) >= 1:
                    instance = self.text_to_instance(
                        ...
                        label=line['label'],
                        seed_col0=line['seed_col0'],
                        label_col0=line['label_col0'],
                        dtypes=line['dtypes']
                    )
    ...
    def text_to_instance(self, table_id, table_header, table_data, n_rows, n_cols, fname, label_idx, label, seed_col0, label_col0, dtypes) -> Instance:
        fields = {
