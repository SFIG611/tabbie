...
class TableEmbedder(Model):
    ...
    def get_labels(self, table_info, bs, n_rows, n_cols):
        ...
        for k, one_info in enumerate(table_info):
            if 'header_labels' in one_info:
                for label_idx in one_info['header_labels']:
    ...
    def forward(self, table_info: Dict[str, str],
        ...
        prob_tables = prob_tables_cls[:, 1, 0, :]

        table_labels = torch.zeros((bs), device=self.device)
        for k, one_info in enumerate(table_info):
            table_labels[k] = one_info['table_labels'][0][0]  # support bin classification only

        # get labels
        # header_labels, cell_labels = self.get_labels(table_info, bs, n_rows, n_cols)
        # self.get_table_labels()
        # header_labels_1d, cell_labels_1d = header_labels.reshape(-1), cell_labels.reshape(-1)
        table_labels_1d = table_labels.reshape(-1)
        # header_mask_1d, cell_mask_1d = table_mask[:, 0, :].reshape(-1), table_mask[:, 1:, :].reshape(-1)
        prob_tables_pos, prob_tables_nega = prob_tables[:, 1], prob_tables[:, 0]
        prob_headers_pos, prob_cells_pos, prob_cells_nega = prob_headers[:, :, 1], prob_cells[:, :, :, 1], prob_cells[:, :, :, 0]
        prob_headers_pos_1d, prob_cells_pos_1d, prob_cells_nega_1d = prob_headers_pos.reshape(-1), prob_cells_pos.reshape(-1), prob_cells_nega.reshape(-1)
        ...
        table_loss = self.loss_func(prob_tables_pos, table_labels_1d)
        ...
        self.metrics['accuracy'](prob_tables, table_labels_1d)  # no mask

        # h_loss_weight = 0.0
        # loss = cell_loss * (1.0-h_loss_weight) + header_loss * h_loss_weight
        output_dict = {'loss': table_loss}
